...
export const fetchSubstations = async (): Promise<Substation[]> => {
  try {
    const response = await invoke<Substation[]>('get_substations_n');
...
export async function loadSubstations(): Promise<FetchStatus> {
  try {
    const result = await invoke<FetchStatus>('load_substations_n');
...
export async function getSubstationById(
  ...
  try {
    return await invoke<Substation | null>('get_substation_by_id_n', { id });
  } catch (error) {
...
export async function searchSubstations(
  ...
  try {
    return await invoke<PaginatedResponse<Substation[]>>('search_substations_n', {
      query,
      pagination,
      search_fields,
    });
