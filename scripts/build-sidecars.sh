...
case $OS in
    "Linux")
        ...
        pyinstaller -c -F --clean --specpath "$TEMP_SPEC_DIR" --name powsybl-x86_64-unknown-linux-gnu --distpath src-tauri/app/binaries/ src-sidecars/powsybl/main.py
        ;;
    "MacOS")
        echo "Executing MacOS-specific commands..."
        setup_venv
        echo "Running PyInstaller for macOS..."
        pyinstaller -c -F --clean --specpath "$TEMP_SPEC_DIR" --name powsybl-x86_64-apple-darwin --distpath src-tauri/app/binaries/ src-sidecars/powsybl/main.py
        ;;
    "Windows")
        echo "Executing Windows-specific commands..."
        setup_venv
        echo "Running PyInstaller for Windows..."
        pyinstaller -c -F --clean --specpath "$TEMP_SPEC_DIR" --name powsybl-x86_64-pc-windows-msvc --distpath src-tauri/app/binaries/ src-sidecars/powsybl/main.py
        ;;
    *)
