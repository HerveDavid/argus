...
let eventBuffer: ScadaMessage[] = [];
let batchTimeoutId: number | null = null;
let isProcessing = false;
let lastFlushTime = 0;

// Atoms
export const eventsAtom = atom<ScadaMessage[]>([]);

export const addScadaMessageAtom = atom(
  null,
  (get, set, newScadaMessage: ScadaMessage) => {
    eventBuffer.push(newScadaMessage);
    ...
    const flushBuffer = () => {
      ...
      const currentScadaMessages = get(eventsAtom);
      const bufferLength = eventBuffer.length;

      let newScadaMessages: ScadaMessage[];

      if (currentScadaMessages.length + bufferLength <= MAX_EVENTS) {
        newScadaMessages = [...currentScadaMessages, ...eventBuffer];
      } else {
        ...
        newScadaMessages = [
          ...currentScadaMessages.slice(-fromCurrent),
          ...eventBuffer.slice(-fromBuffer),
        ];
      }

      set(eventsAtom, newScadaMessages);
