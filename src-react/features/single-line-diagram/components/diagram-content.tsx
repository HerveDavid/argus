...
export const DiagramContent: React.FC<DiagramContentProps> = ({
  ...
  const initializeSvg = useCallback(async (svgString: string) => {
  ...
  const enrichElementData = useCallback((svgString: string, metadata?: SldMetadata): ElementData[] => {
  ...
  const updateSvgWithDataJoin = useCallback((newSvgString: string, metadata?: SldMetadata) => {
    ...
    updateTransition.filter((d: ElementData) => d.d && d.type === 'wire')
    ...
    updateTransition.filter((d: ElementData) => d.type === 'breaker' || d.type === 'disconnector')
      .filter(function() {
        return this instanceof SVGElement;
      })
      .style('opacity', 0.6)
    ...
    updateTransition.filter((d: ElementData) => d.type === 'feeder' && (d.powerActive !== undefined || d.powerReactive !== undefined))
  ...
  }, [diagramData?.svg, diagramData?.metadata, initializeSvg, updateSvgWithDataJoin]);

  // Toggle breaker optimisé
  const toggleBreaker = useCallback((breakerId: string, isClosed: boolean) => {
  ...
  const handleContextMenuTrigger = useCallback((e: React.MouseEvent<SVGSVGElement>) => {
    const target = e.target as SVGElement;
    let element: SVGElement | null = target;

    // Recherche de l'élément parent avec ID
    while (element && !element.id && element.parentElement instanceof SVGElement) {
