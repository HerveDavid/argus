...
const reloadDiagramActor = fromPromise(
  async ({
    ...
    const program = Effect.gen(function* () {
      ...
      // Appel à getSingleLineDiagram qui retourne SingleLineDiagramResponse
      const response = yield* powsyblClient.getSingleLineDiagram({
        element_id: lineId,
      });
      ...
      if (!response.success) {
        return yield* Effect.fail(
          new Error(response.error || 'Failed to generate diagram'),
        );
      }

      // Transformer la réponse en SldDiagram
      const sldDiagram: SldDiagram = {
        id: lineId,
        element_id: response.element_id || lineId,
