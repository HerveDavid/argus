...
// Effect pour récupérer les équipements
const getEquipmentEffect = (params: SubstationQueryParams) =>
  Effect.gen(function* () {
...
// Effect pour obtenir les pays disponibles
const getAvailableCountriesEffect = () =>
  Effect.gen(function* () {
    const powsyblClient = yield* PowsyblClient;
...
// Effect pour obtenir les TSOs disponibles
const getAvailableTSOsEffect = () =>
  Effect.gen(function* () {
    const powsyblClient = yield* PowsyblClient;
...
// Effect pour obtenir les statistiques
const getEquipmentStatsEffect = () =>
  Effect.gen(function* () {
    const powsyblClient = yield* PowsyblClient;
...
// Rx pour les pays - cache statique
const getAvailableCountriesRx = rxRuntime.rx(
  Effect.fn(function* () {
    return yield* getAvailableCountriesEffect();
  }),
);

// Rx pour les TSOs - cache statique
const getAvailableTSOsRx = rxRuntime.rx(
  Effect.fn(function* () {
    return yield* getAvailableTSOsEffect();
  }),
);

// Rx pour les statistiques - cache statique
const getEquipmentStatsRx = rxRuntime.rx(
  Effect.fn(function* () {
    return yield* getEquipmentStatsEffect();
  }),
);

// Hook principal pour obtenir les équipements avec react-query pour la pagination
export const useEquipment = (
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<SubstationQueryResponse> => {
      return runtime.runPromise(getEquipmentEffect(params));
    },
    // Options par défaut
...
// Hook pour obtenir la liste des pays disponibles avec Effect-RX
export const useAvailableCountries = () => {
  const result = useRxValue(getAvailableCountriesRx);
...
// Hook pour obtenir la liste des TSOs disponibles avec Effect-RX
export const useAvailableTSOs = () => {
  const result = useRxValue(getAvailableTSOsRx);
...
// Hook pour obtenir les statistiques globales avec Effect-RX
export const useEquipmentStats = () => {
  const result = useRxValue(getEquipmentStatsRx);
