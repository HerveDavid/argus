...
export interface GameMasterDataPoint {
  id: string;
  graphical_id: string;
  value: any;
  timestamp?: number;
...
export function createGameMasterDataPoint(
  output: GameMasterOutput,
  message: ScadaMessage,
): GameMasterDataPoint | null {
...
// Fonction utilitaire pour créer plusieurs GameMasterDataPoints à partir de listes
export function createGameMasterDataPoints(
  outputs: GameMasterOutput[],
  ...
  return outputs
    .map((output) => {
      ...
      return message ? createGameMasterDataPoint(output, message) : null;
    })
    .filter((dataPoint): dataPoint is GameMasterDataPoint => dataPoint !== null);
}
