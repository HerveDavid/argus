...
class InjectionsRepository(BaseRepository):
    ...
    def load_data(self) -> None:
        try:
            ...
            self._conn.execute(f"""
                ...
                (id, type, voltage_level_id, node, bus_breaker_bus_id, 
                 connected, bus_id, p, q, i)
                SELECT id, type, voltage_level_id, node, bus_breaker_bus_id, 
                       connected, bus_id, p, q, i
    ...
    def get_network_data(self) -> pd.DataFrame:
        return self._network.get_injections(all_attributes=True)