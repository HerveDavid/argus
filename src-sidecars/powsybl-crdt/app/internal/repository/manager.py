...
class Manager:
    ...
    def register_custom_repository(self, name: str, repository_class: type, custom_data: Any) -> None:
        if not issubclass(repository_class, BaseRepository):
            raise ValueError("Repository must inherit from BaseRepository")

        self._repositories[name] = repository_class(self._conn, self._network, custom_data)
