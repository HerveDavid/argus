...
use crate::game_master::entities::{QueryResponse, ScadaOutput, ScadaQuery};
use crate::game_master::error::Error;
...
pub async fn get_scada_outputs(
    ...
    let query_body = ScadaQuery {
        query: "SELECT * FROM game_master_outputs WHERE dynawo_id = ANY(SELECT unnest(?))"
            .to_string(),
