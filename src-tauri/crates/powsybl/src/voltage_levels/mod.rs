...
use super::utils::send_zmq_request;
...
pub async fn get_voltage_levels(
    state: State<'_, PowsyblState>,
    ...
    // Update the state
    {
        ...
        app_state.voltage_levels.clear();
...
#[tauri::command(rename_all = "snake_case")]
pub async fn load_voltage_levels(state: State<'_, PowsyblState>) -> PowsyblResult<FetchStatus> {
...
pub fn get_paginated_voltage_levels(
    ...
    let total = app_state.voltage_levels.len();
...
pub fn get_voltage_levels_by_id(
    ...
    let voltage_level = app_state.voltage_levels.get(&id).cloned();
