...
use crate::scada::entities::{LegacyMessage, ScadaMessage, ScadaOutput, TsTmMessage};
use crate::sessions::state::SessionState;
use crate::utils::tasks::CancellableTask;

use super::get_streams;
use super::parsing::{ParseResult, ParsingConfig, ScadaMessageParser};
...
pub async fn create_task_feeder_v2(
    ...
    CancellableTask::new({
        let parser = ScadaMessageParser::new(ParsingConfig::default());

        let streams = get_streams(nats_client, session_client, metadata)
            .await
            .unwrap();
        let mut combined = select_all(streams);

        move |cancellation_token| async move {
            while let Some((topic, output, msg)) = combined.next().await {
                if cancellation_token.is_cancelled() {
                    info!("Cancellation received for feeder '{}'", topic);
                    return;
                }

                match parser.parse_message(&output, &msg.payload, &topic) {
                    ParseResult::Success(message) => {
                        debug!("Successfully parsed message for '{}'", topic);
                        if let Err(e) = channel.send(message) {
                            warn!("Failed to send message to channel for '{}': {:?}", topic, e);
                        }
                    }
                    ParseResult::PartialSuccess(message, warnings) => {
                        debug!(
                            "Parsed message with warnings for '{}': {:?}",
                            topic, warnings
                        );
                        if let Err(e) = channel.send(message) {
                            warn!("Failed to send message to channel for '{}': {:?}", topic, e);
                    ...
                    ParseResult::Failed(error) => {
                        error!("Failed to parse message for '{}': {:?}", topic, error);
                    }
                }
            }
        }
